let currentMode = 'normal';
        ...
        let amplitudeParams = {
            volume: 0.7,    // Уменьшено для предотвращения клиппинга
            saturation: 0.0
        };
            ...
            for (let channel = 0; channel < buffer.numberOfChannels; channel++) {
                ...
                for (let i = 0; i < buffer.length; i++) {
                    ...
                    if (Math.abs(sample) > threshold - knee) {
                        ...
                        if (x > threshold + knee) {
                            sample = Math.sign(sample) * (threshold + (x - threshold) / (1 + amount * 2));
                        } else {
        ...
        function generateKickAndDrawStaticOscilloscope() {
            ...
            if (clickParams.enabled && clickParams.clickAmount > 0) {
                ...
                clickGain.gain.exponentialRampToValueAtTime(0.0001, startTime + clickParams.clickDuration);
            ...
            offlineCtx.startRendering().then(renderedBuffer => {
                ...
                if (amplitudeParams.saturation > 0) {
                    finalBuffer = applySoftSaturation(renderedBuffer, amplitudeParams.saturation);
                }
                ...
                drawStaticOscilloscope(audioData, totalDuration, freqStart, freqEnd, isClipping);
        ...
        function drawStaticOscilloscope(audioData, duration, freqStart, freqEnd, mode, isClipping) {
            ...
            ctx.fillText(
                `Freq: ${freqStart.toFixed(0)}Hz → ${freqEnd.toFixed(0)}Hz | Dur: ${duration.toFixed(2)}s`,
                canvas.width / 2,
                20
            );
            ctx.fillText(
                `Pitch: ${frequencyParams.pitchDecay.toFixed(2)} | Click: ${clickParams.clickAmount.toFixed(1)}`,
                canvas.width / 2,
                40
            );
            ctx.fillText(
                `Click Freq: ${clickParams.clickFreq}Hz | Vol: ${amplitudeParams.volume.toFixed(1)}`,
                canvas.width / 2,
                60
            );
        ...
        function createAudioControls(audioBuffer) {
            ...
            playBtn.style.color = currentMode === 'eight08' ? '#ff0' : '#0f0';
            playBtn.style.border = `1px solid ${currentMode === 'eight08' ? '#ff0' : '#0f0'}`;
            ...
            downloadBtn.style.color = currentMode === 'eight08' ? '#ff0' : '#0f0';
            downloadBtn.style.border = `1px solid ${currentMode === 'eight08' ? '#ff0' : '#0f0'}`;
        ...
        function createSliderPanel(panelId, title, paramsConfig, paramsObject, onChange) {
        ...
        function initializeControls() {
            ...
            // Frequency controls
            createSliderPanel('frequency-panel', 'Frequency Settings', [
                { name: 'minFreq', label: 'Frequancy', min: 20, max: 200, step: 1, value: 100, unit: 'Hz' },
                { name: 'pitchDecay', label: 'Pitch Decay', min: 0.05, max: 1, step: 0.05, value: 0.1, precision: 2 }
            ], frequencyParams);
            
            // Amplitude controls
            createSliderPanel('amplitude-panel', 'Amplitude Settings', [
                { name: 'volume', label: 'Volume', min: 0.1, max: 0.8, step: 0.1, value: 0.7, precision: 1 },
                { name: 'saturation', label: 'Saturation', min: 0.0, max: 0.8, step: 0.1, value: 0.0, precision: 1 }
            ], amplitudeParams);
            
            // Duration controls
            createSliderPanel('duration-panel', 'Duration Settings', [
                { name: 'duration', label: 'Duration', min: 0.1, max: 3.0, step: 0.1, value: 0.5, unit: 's', precision: 1 },
            ], durationParams);
            
            // ADSR controls
            createSliderPanel('adsr-panel', 'ADSR Envelope', [
                { name: 'attack', label: 'Attack', min: 0.001, max: 0.05, step: 0.001, value: 0.005, unit: 's', precision: 3 },
                { name: 'decay', label: 'Decay', min: 0.01, max: 0.5, step: 0.01, value: 0.1, unit: 's', precision: 2 },
                { name: 'sustain', label: 'Sustain', min: 0.0, max: 0.8, step: 0.1, value: 0.3, precision: 1 },
                { name: 'release', label: 'Release', min: 0.01, max: 1.0, step: 0.01, value: 0.3, unit: 's', precision: 2 }
            ], adsrParams);
            
           
            
            createSliderPanel('click-panel', 'Click Settings', [
                { name: 'clickAmount', label: 'Click Amount', min: 0.0, max: 1.0, step: 0.1, value: 0.3, precision: 1 },
                { name: 'clickFreq', label: 'Click Frequency', min: 500, max: 5000, step: 100, value: 2000, unit: 'Hz' },
                { name: 'clickDuration', label: 'Click Duration', min: 0.005, max: 0.1, step: 0.005, value: 0.02, unit: 's', precision: 3 }
            ], clickParams);
            
           
            
            // Generate button
            document.getElementById('generate-btn').addEventListener('click', generateKickAndDrawStaticOscilloscope);
        }
